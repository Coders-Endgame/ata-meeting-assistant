...
class ZoomBot:
    def __init__(self, meeting_url, bot_name="ASR Bot", session_id=None):
        ...
        if supabase_url and supabase_key:
            ...
            logging.warning("Supabase credentials not found. Transcripts will only be saved locally.")
        ...
        self.server_url = os.environ.get('SERVER_URL', 'http://host.docker.internal:3001')
        ...
        logging.info(f"Model Config: {self.model_size} on {self.device} ({self.compute_type})")
        ...
        try:
             ...
             logging.error(f"Failed to load model on {self.device}, falling back to cpu/small: {e}")
    ...
    async def start_browser(self, headless=True):
        ...
        self.browser = await self.playwright.chromium.launch(
            headless=headless, 
            args=args,
            ignore_default_args=["--mute-audio"] # Ensure we don't mute by default if PW does that
        )
        ...
        await context.grant_permissions(['microphone', 'camera']) # We'll mute them later
        ...
        # TOS ("Agree")
        try:
            agree_btn = self.page.locator('button:has-text("Agree"), button:has-text("I Agree")').first
    ...
    async def check_meeting_ended(self):
        ...
        try:
            ...
            if current_url and '/wc/' not in current_url and 'zoom' not in current_url.lower():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default="ATA Smart Meeting Assistant", help="Bot Name")
    parser.add_argument("--session-id", type=str, help="Session ID for database storage")
    ...
    if args.url:
        ...
        url = input("Enter Zoom Meeting URL: ")
